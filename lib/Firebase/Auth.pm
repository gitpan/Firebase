...
B<NOTE:> As of this writing the debug flag has no effect on web service calls through the REST interface (which this module uses). However, if you use the auth token generated by this module through the Javascript interface then you'll see the extra debug in the console.log in your browser.
